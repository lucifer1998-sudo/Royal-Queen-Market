...
@section('profile-content')
    ...
    <h1 class="my-3 text-white">Bitmessage</h1>
    <hr>
    <p class="my-3 text-white">Service to forward all your notifications from marketplace to your Bitmessage address</p>
    <p class="text-white">Service staus: @if($enabled) <span class="badge badge-success">Enabled</span> @else <span class="badge badge-danger">Disabled</span> @endif</p>
    ...
    @if($user->bitmessage_address == null)
        <h4 class="text-white">Add Bitmessage address</h4>
    @else
        <h4 class="text-white">Change Bitmessage address</h4>
        <p class="text-muted text-white">Current address: {{$user->bitmessage_address}}</p>
    @endif
    <hr>

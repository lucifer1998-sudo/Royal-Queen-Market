<div class="relative bg-rqm-dark flex flex-wrap justify-center rounded shadow w-full h-full">
    ...
    <div class="z-20 p-10 w-full">
                ...
                <div class="grid grid-flow-col grid-cols-3 grid-rows-1 gap-4 w-full">
                    <div class="flex">
                        <input type="number" name="days"
                               class="bg-rqm-dark border border-rqm-yellow-darkest p-2 rounded text-rqm-yellow w-1/2 w-full"
                               placeholder="Older than" aria-label="Days">
                        <button name="type" value="orlder_than_days" type="submit"
                                class="bg-rqm-yellow-dark font-extrabold ml-1 mr-5 p-2 rounded-sm text-base text-rqm-dark w-1/2 w-full">
                            Remove
                        </button>
                    </div>
                    <div>
                        <button name="type" value="solved" type="submit"
                                class="bg-rqm-yellow-dark font-extrabold p-2 rounded-sm text-rqm-dark text-base w-full">
                            Remove solved tickets
                        </button>
                    </div>
                    <div>
                        <button name="type" value="all" type="submit"
                                class="bg-rqm-yellow-dark font-extrabold p-2 rounded-sm text-rqm-dark text-base w-full">
                            Remove all tickets
                        </button>
                    </div>
        ...
        <form method="POST" action="{{ route('admin.ticket.action') }}">
            {{ csrf_field() }}
                ...
                @foreach($tickets as $index => $ticket)
                    <tr class="@if(!($index % 2)) bg-rqm-light @endif">
                        <td class="border-gray-600 border-r px-2 py-2 text-gray-400">
                            <input type="checkbox" class="checkbox-form" name="ticket_uuid[]"
                                   value="{{ $ticket -> id }}">
                            ...
                            @if($ticket -> solved)
                                <span
                                    class="bg-rqm-yellow-darkest font-black ml-3 px-2 rounded-full text-rqm-dark text-xs">Solved</span>
                            @else
                                @if($ticket -> answered)
                                    <span
                                        class="bg-rqm-yellow-dark font-black ml-3 px-2 rounded-full text-rqm-dark text-xs">Answered</span>
                                @endif
                            @endif
                        </td>
                        <td class="border-gray-600 border-r px-2 py-2 text-gray-400">
            ...
            @if ($tickets -> count() > 0)
                <div class="w-full text-center my-5">
                    <button class="bg-rqm-yellow-dark font-extrabold ml-1 mr-5 p-2 rounded-sm text-base text-rqm-dark"
                            type="submit" name="resolve" value="1">Resolve
                    </button>
                    <button class="bg-rqm-yellow-dark font-extrabold ml-1 mr-5 p-2 rounded-sm text-base text-rqm-dark"
                            type="submit" name="read" value="1">Read
                    </button>
                    <button class="bg-rqm-yellow-dark font-extrabold ml-1 mr-5 p-2 rounded-sm text-base text-rqm-dark"
                            type="submit" name="delete" value="1">Delete
                    </button>
                </div>
            @endif
        </form>

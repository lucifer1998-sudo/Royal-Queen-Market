            ...
            <form action="{{route('admin.user.edit.info',$user->id)}}" method="post">
                ...
                <div class="flex w-full pt-2">
                    <div class="pr-5 rounded w-1/2">
                        ...
                        <div><input type="text" name="id" id="id" class="bg-rqm-dark border border-rqm-yellow-darkest p-2 rounded text-rqm-yellow w-full" value="{{$user->id}}" readonly></div>
                    </div>
                    <div class="pl-5 rounded w-1/2">
                        <div class="font-black text-rqm-yellow">Referral code</div>
                        <div><input type="text" name="referral_code" id="referral_code" class="bg-rqm-dark border border-rqm-yellow-darkest p-2 rounded text-rqm-yellow w-full" value="{{$user->referral_code}}"></div>
                    </div>
                </div>
                <div class="flex pt-1 w-full">
                    <div class="pr-5 rounded w-1/2">
                        <div class="font-black text-rqm-yellow">Username</div>
                        <div><input type="text" name="username" id="username" class="bg-rqm-dark border border-rqm-yellow-darkest p-2 rounded text-rqm-yellow w-full" value="{{$user->username}}"></div>
                    </div>
                    <div class="pl-5 rounded w-1/2">
                        <div class="font-black text-rqm-yellow">Last login</div>
                        <div><input type="text" name="last_login" id="last_login" class="bg-rqm-dark border border-rqm-yellow-darkest p-2 rounded text-rqm-yellow w-full" value="{{$user->lastSeenForHumans()}}" readonly></div>
                    </div>
                </div>
